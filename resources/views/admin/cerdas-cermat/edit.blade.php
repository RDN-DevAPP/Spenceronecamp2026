...
@section('title', 'Edit Soal Cerdas Cermat - Admin')
        ...
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-white shadow overflow-hidden sm:rounded-lg">
                <div class="px-4 py-5 sm:px-6 border-b border-gray-200">
                    <h3 class="text-lg leading-6 font-medium text-gray-900">Edit Soal</h3>
                </div>
                <div class="px-4 py-5 sm:p-6">
                    <form action="{{ route('admin.cerdas-cermat.update', $question->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-4">
                            ...
                            <select name="type" id="type" onchange="toggleOptions()"
                                ...
                                <option value="Pilihan Ganda" {{ $question->type == 'Pilihan Ganda' ? 'selected' : '' }}>
                                    Pilihan Ganda</option>
                                <option value="Isian Singkat" {{ $question->type == 'Isian Singkat' ? 'selected' : '' }}>Isian
                                    Singkat</option>
                                <option value="Uraian" {{ $question->type == 'Uraian' ? 'selected' : '' }}>Uraian</option>
                            </select>
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="question" id="question" rows="3" required
                                class="mt-1 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md focus:ring-scout-primary focus:border-scout-primary">{{ old('question', $question->question) }}</textarea>
                        </div>

                        <div id="options-container" class="mb-4 {{ $question->type != 'Pilihan Ganda' ? 'hidden' : '' }}">
                            ...
                            @foreach(['a', 'b', 'c', 'd', 'e'] as $opt)
                                <div class="flex items-center mb-2">
                                    ...
                                    <input type="text" name="options[{{ $opt }}]" value="{{ $options[$opt] ?? '' }}"
    ...
    <script>
        function toggleOptions() {
            ...
            if (type === 'Pilihan Ganda') {
                ...
                // Only make required if not hidden, though server side validation handles it mainly
