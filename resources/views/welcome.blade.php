...
@section('content')
    ...
    <div class="py-12 sm:py-16 bg-white">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <!-- Score Table/Cards -->
            <div class="card-scout rounded-2xl overflow-hidden shadow-lg">
                <!-- Desktop Table -->
                <div class="hidden md:block overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-scout-primary">
                            <tr>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            <tr v-for="regu in allRegu" :key="regu.id" class="hover:bg-scout-surface transition-colors">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        <span v-if="regu.rank === 1" class="text-2xl">🥇</span>
                                        <span v-else-if="regu.rank === 2" class="text-2xl">🥈</span>
                                        <span v-else-if="regu.rank === 3" class="text-2xl">🥉</span>
                                        <span v-else class="text-lg font-bold text-gray-700">@{{
                                            regu.rank }}</span>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="text-base font-bold text-gray-900">@{{ regu.nama_regu }}
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span v-if="regu.jenis === 'putra'"
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span
                                        class="px-3 py-1 inline-flex text-sm leading-5 font-semibold rounded-full bg-scout-accent text-scout-primary">
                                        @{{ regu.nomor_regu }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    <div class="text-2xl font-extrabold text-scout-primary">
                                        @{{ regu.total_nilai.toFixed(2) }}
                                    </div>
                                </td>
                            </tr>
                        </tbody>
                ...
                <div class="md:hidden space-y-4 p-4">
                    <div v-for="regu in allRegu" :key="regu.id"
                        class="bg-scout-surface rounded-xl p-4 border-2 border-scout-secondary/30 hover:border-scout-primary transition-all">
                        <div class="flex items-start justify-between mb-3">
                            <div class="flex items-center">
                                <span v-if="regu.rank === 1" class="text-3xl mr-3">🥇</span>
                                <span v-else-if="regu.rank === 2" class="text-3xl mr-3">🥈</span>
                                <span v-else-if="regu.rank === 3" class="text-3xl mr-3">🥉</span>
                                <span v-else class="text-xl font-bold text-gray-700 mr-3">#@{{ regu.rank
                                    }}</span>
                                <div>
                                    <h3 class="text-lg font-bold text-gray-900">@{{ regu.nama_regu }}
                                    </h3>
                                    <div class="flex items-center gap-2 mt-1">
                                        ...
                                        <span v-else
                                            class="px-2 py-1 text-xs font-semibold rounded-full bg-pink-100 text-pink-800">
                                            Putri
                                        </span>
                                        <span
                                            class="px-2 py-1 text-xs font-semibold rounded-full bg-scout-accent text-scout-primary">
                        ...
                        <div class="mt-3 pt-3 border-t border-gray-200">
                            <div class="flex items-center justify-between">
                                ...
                                <span class="text-2xl font-extrabold text-scout-primary">
                                    @{{ regu.total_nilai.toFixed(2) }}
                                </span>
                            </div>
                        </div>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-10 sm:mb-12">
                ...
                <div class="flex flex-wrap justify-center gap-6">
                    ...
                    <div
                        class="card-scout rounded-2xl p-6 sm:p-8 w-full max-w-md hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1 cursor-pointer"
                        ...
                        <div class="bg-white rounded-xl p-6 sm:p-8 flex items-center justify-center h-32 sm:h-40">
                            @if($sponsor->logo)
                                <img src="{{ Storage::url($sponsor->logo) }}" alt="{{ $sponsor->name }}" class="max-h-full max-w-full object-contain">
                            @else
            ...
            <div class="mb-8 sm:mb-10">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
                    ...
                    <div
                        class="card-scout rounded-xl p-4 sm:p-6 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 cursor-pointer"
                        {{ $sponsor->website_url ? 'onclick=window.open("' . $sponsor->website_url . '")' : '' }}>
            ...
            <div>
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-3 sm:gap-4">
                    ...
                    <div
                        class="card-scout rounded-lg p-3 sm:p-4 hover:shadow-lg transition-all duration-300 transform hover:-translate-y-1 cursor-pointer"
                        {{ $sponsor->website_url ? 'onclick=window.open("' . $sponsor->website_url . '")' : '' }}>
...
@push('scripts')
    <script>
        window.pageData = function () {
            ...
            const targetDate = new Date('2026-04-24T13:30:00').getTime();
