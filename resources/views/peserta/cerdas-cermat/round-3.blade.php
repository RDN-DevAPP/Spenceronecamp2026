...
@section('title', 'Babak Final - Cerdas Cermat')
        ...
        <div class="bg-white rounded-lg shadow-xl overflow-hidden border border-scout-secondary">
            <div
                ...
                <div class="text-center sm:text-left">
                    <h2 class="text-lg sm:text-xl font-bold text-white">Babak Final: Uraian</h2>
                    <p class="text-white/90 text-xs sm:text-sm">Jelaskan jawaban Anda dengan detail.</p>
                </div>
            ...
            <div class="p-4 sm:p-8">
                <form action="{{ route('peserta.cerdas-cermat.round-3.submit') }}" method="POST" id="examForm">
                    @csrf
                    ...
                    @foreach($questions as $index => $question)
                        <div class="mb-6 sm:mb-8 border-b border-gray-100 pb-6 last:border-0">
                            <div class="flex items-start mb-4">
                                ...
                                <div
                                    class="flex-shrink-0 w-6 h-6 sm:w-8 sm:h-8 flex items-center justify-center bg-scout-accent text-scout-primary text-sm sm:text-base font-bold rounded-full mr-3">
                                    {{ $index + 1 }}
                                </div>
                            ...
                            <div class="ml-9 sm:ml-11">
                                <textarea name="answers[{{ $question->id }}]" rows="4"
                                    class="w-full rounded-md border-gray-300 shadow-sm focus:border-scout-primary focus:ring focus:ring-scout-primary focus:ring-opacity-50"
    ...
    @push('scripts')
        <script>
            ...
            function autoSubmitForm() {
                ...
                const storagePrefix = 'ans_r3_s{{ $session->id }}_';
                textareas.forEach(textarea => {
                    localStorage.removeItem(storagePrefix + textarea.name);
                });
                localStorage.removeItem('round_3_submitted');
            ...
            // Persistence Logic for Round 3
            document.addEventListener('DOMContentLoaded', function () {
                const textareas = document.querySelectorAll('textarea');
                const storagePrefix = 'ans_r3_s{{ $session->id }}_';

                textareas.forEach(textarea => {
                    const key = storagePrefix + textarea.name;
